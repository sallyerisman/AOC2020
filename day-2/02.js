...
inputList.forEach((input) => {
  ...
  const maxNumber = Number(numbers[1]);
  
	const letter = Object.keys(input[1]);
	const password = Object.values(input[1]);

	
	let includesLetter = 0;
	for (let i = 0; i < password[0].length; i ++) {
		if ( password[0].charAt(i) === letter[0] ) {
			includesLetter ++;
		}
	}
  
  if (includesLetter >= minNumber && includesLetter <= maxNumber) {
		validPasswords ++;
	}
});
...
inputList.forEach((input) => {
  ...
  const maxNumber = Number(numbers[1]);
	...
	if ( password[0].charAt(minNumber - 1) === letter[0] && password[0].charAt(maxNumber - 1) !== letter[0]) {
		validPasswords ++;
	} else if (password[0].charAt(minNumber - 1) !== letter[0] && password[0].charAt(maxNumber - 1) === letter[0]) {
		validPasswords ++;
	}
});
